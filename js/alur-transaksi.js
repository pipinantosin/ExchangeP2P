...
// ===============================
// OPEN PI WALLET (DEEP LINK)
// ===============================
function openPiWallet(wallet, amount){
    if(!amount || amount <= 0){
        alert("Masukkan jumlah PI terlebih dahulu");
        return;
    }

    const deepLink =
    `pi://wallet/pay?recipient=${wallet}&amount=${amount}`;

    const webLink =
    `https://wallet.minepi.com/pay?recipient=${wallet}&amount=${amount}`;
...
generateBtn.addEventListener("click", () => {
    ...
    if(token === "sidra"){
        ...
        link = `https://www.sidrachain.com/wallets/send?to=${sidraWallet}&amount=${amount}&currency=SDA`;
        generatedLinkEl.onclick = null;
    }
    ...
    if(token === "pi"){
        ...
        link = `https://wallet.minepi.com/pay?recipient=${piWallet}&amount=${amount}`;

        // klik link → coba buka Pi Browser
        generatedLinkEl.onclick = function(e){
            e.preventDefault();
            openPiWallet(piWallet, amount);
        };
    }

    // set href untuk QR & fallback
...
verifyBtn.addEventListener("click", async () => {
    ...
    if(!txHash){ alert("Masukkan hash transaksi"); return; }
...
sendWAButton.addEventListener("click", () => {
    ...
    if(!tx){ alert("Transaksi belum diverifikasi!"); return; }
